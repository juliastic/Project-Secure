...
const INTRO_DIALOGUE := [
	"[offset x=10]>> Hello ... nice to meet you. I'm guessing this is your first day? [i]Press ENTER or RETURN to continue[/i]",
...
const SUPPORTED_COMMANDS := {
	GameProgress.Level.TUTORIAL: [TerminalCommands.MIN, TerminalCommands.CLEAR, TerminalCommands.GRAB_COFFEE, TerminalCommands.HELP, TerminalCommands.BACKSTORY, TerminalCommands.LIST_TERMS, TerminalCommands.EXPLAIN],
		GameProgress.Level.RANSOMWARE_TRIGGER: [TerminalCommands.MIN, TerminalCommands.CLEAR, TerminalCommands.GRAB_COFFEE, TerminalCommands.HELP, TerminalCommands.BACKSTORY, TerminalCommands.LIST_TERMS, TerminalCommands.EXPLAIN], 
...
const BACKSTORY_VALUES = {
	...
	GameProgress.Level.SOCIAL_ENGINEERING: "I think our [i]IDS[/i] has detected an intruder. Some employee must have shared their account information with them. Guess we have to get out the big guns and erase them from our system! Damn cups.",
	GameProgress.Level.EoP: "We've finally gathered enough information about the cup to pursue them. [b]FOR GNOMEREGAN![/b] Sorry about that ... I've been spending too much time playing a silly game. Now I understand why humans are so unproductive."
}

const IN_LEVEL_BACKSTORY_VALUES = {
	GameProgress.Level.RANSOMWARE: {
		0: "Well done. Now get the firewall and the honeypot file ready!",
		1: "Don't worry about the name of the honeypot file. [i]%[/i] is good enough to get the job done.\n[b]NOW SCAN OUR SYSTEM![/b]"
	},
	GameProgress.Level.DDoS: {
		0: "You're ready to go. Go full on defense mode!"
	},
	GameProgress.Level.SOCIAL_ENGINEERING: {
		0: "Good job! Let's find the bad cup who is impersonating an employee."
	},
	GameProgress.Level.EoP: {
		0: "[b]FOR GNOMEREGAN![/b] We're about to catch that damn cup!"
	}
}
...
const EXPLAIN_VALUES = {
	...
	GameProgress.Level.EoP: {
		"EOP": "Elevation of Privileges\nA type of attack where attackers gain access to an internal account and elevate the account's privileges to access information that would otherwise not be accessible to the account.",
		"SOCIAL ENGINEERING": "A type of system attack to either manipulate people into performing actions or accessing confidential information through other people's accounts. Access can be gained in many ways, i.e. through manipulating people into giving the attacker their credentials.",
		"DDOS": "Distributed Denial of Service\nA type of system attack where attackers send a high number of requests to a single server with the goal of overwhelming it and forcing it to go offline.",
		"RANSOMWARE": "A type of malicious software that tries to gain access to a computer, fetch sensitive data and block access.", 
		"IDS": "INTRUSION DETECTION SYSTEM\nIt can be used to detect and alert on suspicious or malicious traffic on the network.", 
		"IPS": "INTRUSION PREVENTION SYSTEM\nIt can automatically block malicious traffic in real-time.",
		"HONEYPOT": "A type of system to deliberately attract attacks. The system looks normal from an attacker's point of view but it is not used. Instead, it watches the actions of the attacker to give an understanding of attack patterns.",
...
func generate_list_terms(level: int) -> String:
	var term_list = "\nI can give you more information on the following terms:"
